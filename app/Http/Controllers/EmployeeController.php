...
class EmployeeController extends Controller
{
    ...
    public function saveEmployee(Request $request)
    {
        ...
        if ($number_user == 0) {
            ...
            $number_user = "MHU" . sprintf("%05d", (int)$number_user + 1);
        }
    ...
    public function editEmployee($id)
    {
        ...
        $emp = Employee::select('employees.*', 'departments.*', 'job_positions.*')
            ->join('departments', 'departments.id','=','employees.department','left')
            ->join('job_positions', 'job_positions.id','=', 'employees.job_position','left')
            ->where('employees.id', $id)
            ->first();
    ...
    public function generatePayslip($id)
    {
        ...
        $emp = DB::table('employee_salaries')
            ->select('employee_salaries.*', 'job_positions.*', 'employees.*','departments.*' )  
            ->join('job_positions', 'job_positions.id', 'employee_salaries.designation')
            ->join('employees', 'employees.id', 'employee_salaries.emp_id')
            ->join('departments','departments.id','employees.department')
